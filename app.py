...
# Configuración archivo Excel y hojas
...
# Crear archivo si no existe con hojas vacías
...
# --- FORMULARIO REGISTRO DE ENTREGAS ---

archivo_excel = EXCEL_FILENAME

# Leer clientes para el formulario entregas
...
st.set_page_config(page_title="Registro de Entregas y Activaciones", layout="wide")
...
# Leer clientes desde el archivo para activaciones
...
if enviado:
    try:
        df_act = pd.read_excel(EXCEL_FILENAME, sheet_name=EXCEL_SHEET_ACTIVACION)
    except:
        df_act = pd.DataFrame()
    ...
    with pd.ExcelWriter(EXCEL_FILENAME, mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
        df_act.to_excel(writer, sheet_name=EXCEL_SHEET_ACTIVACION, index=False)
